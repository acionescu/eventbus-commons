...
public class PeersAgentContext extends GlobalAgentEventNodeContext{
...
//    private EventNodeContext nodeContext;
	...
	super(nodeContext,peersManager);
//	this.peersManager = peersManager;
//	this.nodeContext = nodeContext;
    }

    public FilteringEventProcessor getBeforePostFilter() {
	return getPeersManager().getBeforePostFilter();
    }

    public void postEvent(Event event) {
	getNodeContext().postEvent(event);
    }
    
    public EventNodeLogger logger() {
	return getNodeContext().getLogger();
    }
    
    public boolean isDebugEnabled() {
	return getNodeContext().getLogger().isDebugEnabled();
    }
    
    public RemotePeerManager getRemotePeer(RemotePeerDataContext data) {
	 return getPeersManager().addRemotePeer(data);
    }
