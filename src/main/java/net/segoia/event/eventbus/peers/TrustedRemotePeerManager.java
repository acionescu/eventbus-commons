    ...
    public void generateNewSession() throws PeerSessionException {
        ...
        if (sessionData != null) {
            ...
            EventNodeSecurityManager securityManager = getNodeContext().
                    getSecurityManager();
            try {
                securityManager.buildSessionFromPlainData(getPeerContext(),
                        sessionData);
            } catch (CommOperationException e) {
                throw new PeerSessionException(
                        "Failed to generate session from known session info",
                        e);
            }
        } else {
 ...
 /* we'll also neet the session comm manager for this */
    ...
    public void startNewPeerSession() throws CommOperationException {
        ...
        if (prevSessionData != null) {
            ...
            getNodeContext().getSecurityManager().storeIdentityLinkFullData(
                    trustData);
    ...
    protected SessionKeyPlainData getPlainSessionKeyData() {
        ...
        final CryptoHelper cryptoHelper = getNodeContext().getSecurityManager().
                getCryptoHelper();
        SessionKeyPlainData newSessionData = new SessionKeyPlainData(sessionKey.
                getSessionId(), cryptoHelper.base64Encode(sessionKey.
                        getKeyBytes()), sessionKey.getKeyDef(), cryptoHelper.
                base64Encode(sessionKey.getIv()));
        return newSessionData;
    }

    @Override
    protected void sendNewSessionInfoToPeer(SessionInfo sessionInfo) {
        forwardToPeer(new StartTrustedCommSessionEvent(
                new StartTrustedCommSessionData(sessionInfo)));
    }
    ...
    public void onReady() {
        ...
        postEvent(new TrustedCommSessionEstablishedEvent(
                new TrustedCommSessionEstablishedData()));
    }
    ...
    private static final PeerManagerState INIT_COMM_SESSION = new PeerManagerState() {
        ...
        protected void registerPeerEventHandlers() {
            registerPeerEventProcessor(TrustedCommSessionAcceptedEvent.class,
                    (c) -> {
        ...
        private void handleCommSessionAccepted(PeerEventContext<TrustedCommSessionAcceptedEvent> c) {
            ...
            try {
                ...
                peerManager.postEvent(new PeerCommErrorEvent(new PeerErrorData(
                        -1, ex.getMessage())));
                return;
            } catch (CommOperationException ex) {
                peerManager.postEvent(new PeerCommErrorEvent(
                        new PeerErrorData(-1, ex.getMessage())));
                return;
                ...
                peerManager.postEvent(new PeerCommErrorEvent(
                        new PeerErrorData(0, t.getMessage())));
                return;
            }
    ...
    private static final PeerManagerState WAIT_FOR_START_SESSION = new PeerManagerState() {
        ...
        @Override
        protected void registerPeerEventHandlers() {
            registerPeerEventProcessor(StartTrustedCommSessionEvent.class,
                    (c) -> {
        ...
        private void handleStartSession(PeerEventContext<StartTrustedCommSessionEvent> c) {
            ...
            TrustedRemotePeerManager peerManager = (TrustedRemotePeerManager) c.
                    getPeerManager();
            ...
            IdentityLinkPrivateData privateData = peerManager.trustData.
                    getPrivateData();
            SessionKeyPlainData prevSessionData = privateData.getSessionData();
            ...
            if (sessionInfo.getSessionData() == null) {
                ...
                if (prevSessionData == null) {
                    TrustedCommSessionClosedEvent closeEvent = new TrustedCommSessionClosedEvent(
                            new TrustedCommSessionClosedData(
            ...
            EventNodeSecurityManager securityManager = peerContext.
                    getNodeContext().getSecurityManager();
            try {
                if (prevSessionData == null) {

                    peerManager.setUpSessionCommManager();
                    securityManager.buildSessionFromSessionInfo(peerContext,
                            sessionInfo);
                } else {
                    securityManager.buildSessionFromPlainData(peerContext,
                            prevSessionData);
                }
            } catch (CommOperationException e) {
            ...
            if (prevSessionData == null || !sessionId.equals(prevSessionData.
                    getSessionId())) {
